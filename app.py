...
warnings.filterwarnings(
    "ignore", category=cryptography.utils.CryptographyDeprecationWarning
)
...
def insert_data(name, category):
    ...
    document = {
        ...
        "Type": category,
        "completed": False,
    }  # Initially set as not completed
...
def fetch_data_by_id(case_id):
    ...
    try:
        document = collection.find_one(
            {"_id": ObjectId(case_id)}, {"name": 1, "Type": 1, "completed": 1}
        )
        if document:
...
def submit():
    ...
    ticket_url = url_for(
        "static", filename=f"tickets/{name}_ticket.txt", _external=True
    )
    mailto_link = (
        f"mailto:dbmsaum@gmail.com?subject=Grievance%20Ticket&body={ticket_url}"
    )
